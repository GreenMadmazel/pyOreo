        ...
        async def wrapp(ore):
            ...
            if not ore.out:
                ...
                if fullsudo and ore.sender_id not in SUDO_M.fullsudos:
                    return await eod(ore, "`Full Sudo User Required...`", time=15)
            ...
            if admins_only:
                if ore.is_private:
                    return await eod(ore, "`Use this in group/channel.`")
                if not (chat.admin_rights or chat.creator):
                    return await eod(ore, "`I am not an admin.`")
            ...
            if groups_only and ore.is_private:
                return await eod(ore, "`Use this in Group/Channel.`")
            try:
                await dec(ore)
                ...
                return await eod(
                    ore, "`Sending media or sticker is not allowed in this chat.`"
                )
            except (BotMethodInvalidError, UserIsBotError):
                return await eod(ore, "This Command Can't be used by Bot!")
                ...
                ftext = (
                    "**Oreo Client Error:** `Forward this to` @UltroidSupport\n\n"
                )
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Oreo Client Error:** `Forward this to` @UltroidSupport\n\n",
                        )
                else:
